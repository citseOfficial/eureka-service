node {
   ...
   stage('Preparation') {
       git branch: 'main' , url: 'https://github.com/citseOfficial/eureka-service'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/eureka-service/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/eureka-service/runDeployment.sh'
            sh '/var/lib/jenkins/workspace/eureka-service/runDeployment.sh'
       }catch (e){
